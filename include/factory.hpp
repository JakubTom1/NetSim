...
public:
    ...
    NodeCollection<Node>::iterator find_by_id(ElementID id) {return std::find_if(container.begin(), container.end(), [id] (const Node& element){return element.get_id()==id;});}
    NodeCollection<Node>::const_iterator find_by_id(ElementID id) const {return std::find_if(container.begin(), container.end(), [id] (const Node& element){return element.get_id()==id;});}
    void remove_by_id(ElementID id) {
        NodeCollection<Node>::iterator find_iter = find_by_id(id);
        if(find_iter != end()){
            container.erase(find_iter);
        }
    }
...
public:
    ...
    void do_package_possing();
    void do_work(Time);

private:
    template<class Node>
    void remove_receiver(NodeCollection<Node>&, ElementID id);
...
std::pair<std::string, std::string> split_string(std::string strng, std::string delimeter);
